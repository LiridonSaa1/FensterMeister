...
export const WindowTypesView: React.FC = () => {
  ...
  // Real-time custom SVG theme controls
  const [frameTheme, setFrameTheme] = useState<'anthracite' | 'white' | 'wood' | 'bronze'>('anthracite');
  const [glassTheme, setGlassTheme] = useState<'blue' | 'clear' | 'lowe' | 'frosted'>('blue');
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative z-10">
          <div className="space-y-1.5 max-w-2xl">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
                {HOUSE_WINDOW_TYPES.length} Window Styles & Schematics
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">
              Residential House Window Types
            </h1>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            >
              ...
              <span>Sync All to Product Catalog ({HOUSE_WINDOW_TYPES.length})</span>
            </button>
            <button
              onClick={() => {
                setSelectedInvoiceId(null);
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by window type, mechanism, room (e.g. Kitchen, Basement, Casement, Bay)..."
              value={searchQuery}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pt-2 border-t border-slate-100">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              { id: 'all', label: 'All Windows (30)' },
              { id: 'operable', label: 'Operable & Ventilation' },
              { id: 'fixed', label: 'Fixed & Panoramic' },
              { id: 'projecting', label: 'Projecting (Bay & Bow)' },
              { id: 'geometric', label: 'Geometric & Sloped' },
              { id: 'folding_pass_through', label: 'Folding & Pass-Through' },
              { id: 'specialty', label: 'Artisan & Leaded Glass' },
              { id: 'architectural', label: 'Arched & Specialty' },
              { id: 'roof_overhead', label: 'Roof & Skylights' },
            ].map((cat) => (
              <button
                key={cat.id}
          ...
          <div className="flex items-center gap-4 text-xs font-medium text-slate-500">
            ...
            <div className="flex items-center gap-1.5">
              ...
              />
              <button
                title="Architectural Dark Bronze"
            ...
            {/* Glass Glazing Tone */}
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] text-slate-400">Glass:</span>
              <button
                title="Sky Blue Low-E"
              ...
              >
                ...
                <div className="bg-slate-50/80 p-6 border-b border-slate-100 relative flex items-center justify-center min-h-[220px] group-hover:bg-slate-50 transition-colors">
                  ...
                  <div className="absolute top-3 left-3 bg-white/95 px-2.5 py-1 rounded-full text-[10px] font-bold text-slate-600 border border-slate-200 shadow-xs">
                    {spec.category.replace('_', ' ').toUpperCase()}
                  </div>
                </div>
                ...
                <div className="p-5 flex-1 flex flex-col space-y-4">
                  <div>
                    ...
                    <p className="text-[11px] text-blue-600 font-medium mt-0.5 flex items-center gap-1">
                      ...
                      <span>{spec.openingMechanism}</span>
                    </p>
                  ...
                  <div className="grid grid-cols-3 gap-2 py-2.5 px-3 bg-slate-50 rounded-xl border border-slate-100 text-[11px]">
                    <div>
                      <span className="text-[10px] text-slate-400 block font-medium">U-Factor</span>
                      <span className="font-bold text-slate-700 font-mono">{spec.energyEfficiency.uFactor}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 block font-medium">SHGC</span>
                      <span className="font-bold text-slate-700 font-mono">{spec.energyEfficiency.shgc}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 block font-medium">Sound STC</span>
                  ...
                  <div className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between text-slate-600">
                      <span className="text-slate-400 text-[11px]">Std. Sizes:</span>
                      <span className="font-medium font-mono text-[11px]">{spec.standardSizes.widthRangeInches}</span>
                    </div>
              ...
              >
                ...
                <div className="space-y-2 text-xs font-mono bg-slate-900/80 p-3.5 rounded-xl border border-slate-800/80">
                  <div className="flex justify-between text-slate-400 text-[11px]">
                    ...
                    <span className="text-sky-300 font-bold">{spec.openingMechanism.split(' ')[0]}</span>
                  </div>
                  <div className="flex justify-between text-slate-400 text-[11px]">
                    <span>Thermal U-Factor:</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-800">
              Technical Comparison Matrix of All 19 Residential House Window Types
            </h3>
            <span className="text-xs text-slate-500">
              Showing {filteredWindows.length} styles
            </span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse">
              <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase tracking-widest font-bold border-b border-slate-200">
                <tr>
                  <th className="py-3.5 px-4">Schematic & Type</th>
                  <th className="py-3.5 px-3">Opening Mechanism</th>
                  <th className="py-3.5 px-3">Ventilation Area</th>
                  <th className="py-3.5 px-3">Thermal U-Factor</th>
                  <th className="py-3.5 px-3">Rain Protection</th>
                  <th className="py-3.5 px-3">Cleaning Ease</th>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400 capitalize">{spec.category}</span>
                          </div>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-2xl max-w-4xl w-full border border-slate-200 shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95">
            ...
            <div className="p-6 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold tracking-tight">{activeWindowModal.name}</h2>
                  <p className="text-xs text-slate-400">Architectural Specifications & Live Dimension Price Estimator</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-5 bg-slate-50 p-6 rounded-2xl border border-slate-200 flex flex-col items-center justify-center space-y-4">
                ...
                <div className="w-full text-center space-y-1">
                  <span className="text-xs font-bold text-slate-800">{activeWindowModal.name}</span>
                  <p className="text-[11px] text-slate-500">{activeWindowModal.openingMechanism}</p>
                </div>
                ...
                <div className="w-full bg-white p-3 rounded-xl border border-slate-200/80 text-xs space-y-1.5">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">U-Factor (Thermal):</span>
                    <span className="font-mono font-bold text-slate-800">{activeWindowModal.energyEfficiency.uFactor}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Acoustic STC:</span>
              ...
              <div className="lg:col-span-7 space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed">{activeWindowModal.description}</p>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3.5">
                  <h4 className="text-xs font-bold text-slate-800 flex items-center justify-between">
                    <span>Configure Dimensions & Materials</span>
                    <span className="text-[11px] text-blue-600 font-mono">Live Price Calculator</span>
                  </h4>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-2">
                  <button
                    onClick={() => {
                      ...
                      prod.name = `${activeWindowModal.name} (${customWidth}"W × ${customHeight}"H) - ${customMaterial}`;
                      addProduct(prod);
                      showNotification(`Added customized ${activeWindowModal.name} to product catalog!`);
